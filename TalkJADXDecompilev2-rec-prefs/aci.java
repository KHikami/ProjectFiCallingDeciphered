...
public final class aci {
    public static final Rect f229a = new Rect();
    private static Class<?> f230b;

    static {
        if (VERSION.SDK_INT >= 18) {
            try {
                f230b = Class.forName("android.graphics.Insets");
            } catch (ClassNotFoundException e) {
            }
        }
    }

    public static Rect m377a(Drawable drawable) {
        if (f230b != null) {
            try {
                Drawable f = jb.m23756f(drawable);
                Object invoke = f.getClass().getMethod("getOpticalInsets", new Class[0]).invoke(f, new Object[0]);
                if (invoke != null) {
                    Rect rect = new Rect();
                    for (Field field : f230b.getFields()) {
    ...
    static void m378b(Drawable drawable) {
        if (VERSION.SDK_INT == 21 && "android.graphics.drawable.VectorDrawable".equals(drawable.getClass().getName())) {
            int[] state = drawable.getState();
            if (state == null || state.length == 0) {
                drawable.setState(afl.f538e);
            } else {
                drawable.setState(afl.f541h);
            }
            drawable.setState(state);
        }
    }

    public static boolean m379c(Drawable drawable) {
            ...
            if (VERSION.SDK_INT >= 17 || !(drawable2 instanceof LayerDrawable)) {
                if (!(drawable2 instanceof DrawableContainer)) {
                    if (!(drawable2 instanceof ji)) {
                        ...
                        drawable2 = ((ji) drawable2).mo3525a();
                    }
                } else {
                    break;
                }
    ...
    public static Mode m376a(int i, Mode mode) {
        switch (i) {
            case 3:
