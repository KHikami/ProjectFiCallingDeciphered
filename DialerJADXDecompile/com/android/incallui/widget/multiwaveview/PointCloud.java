...
public class PointCloud {
    ...
    public int getAlphaForPoint(Point point) {
        ...
        if (hypot < this.glowManager.radius) {
            hypot = this.glowManager.alpha * max(0.0f, (float) Math.pow(Math.cos((((double) hypot) * 0.7853981633974483d) / ((double) this.glowManager.radius)), 10.0d));
        } else {
            hypot = 0.0f;
        }
        float hypot2 = hypot(point.x, point.y) - this.waveManager.radius;
        if (hypot2 < this.waveManager.width * 0.5f && hypot2 < 0.0f) {
            double cos = Math.cos((((double) hypot2) * 0.7853981633974483d) / ((double) this.waveManager.width));
            f = max(0.0f, (float) Math.pow(cos, 20.0d)) * this.waveManager.alpha;
        }
